...
export const handleTree = <T>(data: any[], id?: string, parentId?: string, children?: string): T[] => {
  ...
  // 初始化每个节点的 childrenList
  data.forEach(d => {
    const nodeId = d[config.id];
    childrenListMap[nodeId] = { ...d, [config.childrenList]: [] };
  });

  data.forEach(d => {
    const nodeId = d[config.id];
    const nodeParentId = d[config.parentId];
    ...
    if (
      ...
      tree.push(childrenListMap[nodeId]);
    } else {
      // 否则，找到父节点并加入其 childrenList
      const parent = childrenListMap[nodeParentId];
      if (parent) {
        parent[config.childrenList].push(childrenListMap[nodeId]);
      } else {
        console.warn(`Parent node with id ${nodeParentId} not found for node ${nodeId}`);
      }
    }
  });
